// falloptom-us SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from falloptom-us/index.html · 138430 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "falloptom-us" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Falloptom v1.0.0 · prime 743 · MIT
// Sovereign exam management + US law engine for State Bar-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='falloptom-us';const VERSION='1.0.0';const PRIME=743;const STORE='falloptom-us';const CONFIG_V='falloptom-us@'+VERSION;
const DB_VERSION=1;
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Optometry opinion signed and retained per State Bar 13.5',{id:a.id,examId:a.examId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Falloptom</strong> is a tool for State Bar-regulated US attorneys. It assists with exam management, CDD, document generation, and State Bar Accounts Rules tracking. It is <strong>not regulatory submission or optometry opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign exam management + US law research for 1-10 person State Bar-regulated firms. First, your firm details.</p>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue optometry opinion <span class="meta">State Bar 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>falloptom-uspaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>falloptom-uspractice</strong>. This view shows ledger ref IDs.</p>
...
async function askT3(){
 ...
 const sys='You are a US optometry research assistant for State Bar-regulated attorneys. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated optometry advice — research only. Always say "verify against primary sources".';
...
function exportAll(){downloadJSON('falloptom-us-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,exams:state.exams,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('falloptom-us-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
