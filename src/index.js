...
// Extracted from falloptom-us/index.html · 138349 bytes of source logic
...
// Sovereign exam management + UK law engine for SRA-regulated firms
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Optometry opinion signed and retained per SRA 13.5',{id:a.id,examId:a.examId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Falloptom</strong> is a tool for SRA-regulated UK solicitors. It assists with exam management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or optometry opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign exam management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue optometry opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
async function askT3(){
 ...
 const sys='You are a UK optometry research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated optometry advice — research only. Always say "verify against primary sources".';
